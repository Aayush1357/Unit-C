...
def Onclickstart():    
    def length():
        def choice():
            def calculation(inp,unit_in,unit_out):
                SI = {'nanometer [nm]':10**(-9), 'micrometer [μm]':10**(-6), 'millimeter [mm]':10**(-3), 'centimeter [cm]':10**(-2), 'meter [m]':1, 'kilometer [km]':10**3, 'inch [in]':0.0254, 'foot [ft]':0.3048, 'yard [yd]':0.9144, 'mile [mi]':1609.34}
        ...
        options = ['nanometer [nm]', 'micrometer [μm]', 'millimeter [mm]', 'centimeter [cm]', 'meter [m]', 'kilometer [km]', 'inch [in]', 'foot [ft]', 'yard [yd]', 'mile [mi]']
        ...
        okbtn = Button(length, bg='#C51A4A', fg='#FFFFFF', text='Convert', font=('Footlight MT Light',8,'bold'), command=choice)
    ...
    def mass():
        def choice():
            def calculation(inp,unit_in,unit_out):
                SI = {'microgram [μg]':10**(-9), 'milligram [mg]':10**(-6), 'gram [g]':10**(-3), 'kilogram [kg]':1, 'tonne [t]':10**3, 'ounce [oz]':0.0283495, 'pound [lb]':0.453592, 'US ton':907.185}
                return inp*SI[unit_in]/SI[unit_out]
        ...
        options = ['microgram [μg]', 'milligram [mg]', 'gram [g]', 'kilogram [kg]', 'tonne [t]', 'ounce [oz]', 'pound [lb]', 'US ton']
        ...
        okbtn = Button(mass, bg='#C51A4A', fg='#FFFFFF', text='Convert', font=('Footlight MT Light',8,'bold'), command=choice)
        okbtn.grid(row=2, column=1)
        return
    ...
    win = Toplevel()
    b = PhotoImage(file = 'images.png')
    win.iconphoto(False,b)
    win.deiconify()
    win.resizable(0,0)
